...
# Lab 10: Asynchronously process messages by using Azure Service Bus Queues (Low-Code Path)
...
### Exercise 2: Create a .NET Core project to publish messages to a Service Bus queue (Low-Code Path)
...
1. In the **Open Folder** window, browse to **Allfiles (F):\\Allfiles\\Labs\\10\\Solution\\MessagePublisher**, and then select **Select Folder**.
...
1. Ensure the **queueName** constant is **messagequeue**, matching the name of the Service Bus queue you created earlier in this exercise.

1. Read thru the code comments. Make note of any questions you have about the code.
...
### Exercise 3: Create a .NET Core project to read messages from a Service Bus queue (Low-Code Path)
...
1. In the **Open Folder** window, browse to **Allfiles (F):\\Allfiles\\Labs\\10\\Solution\\MessageReader**, and then select **Select Folder**.
